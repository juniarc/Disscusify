...
function asyncToggleDownVoteComment(commentId) {
    return async (dispatch, getState) => {
        const { authUser, detailThread } = getState();
        dispatch(toggleDownVoteCommentActionCreator({ commentId: commentId, userId: authUser.id }));

        try {
            await api.toggleDownVoteComment({ commentId: commentId, threadId: detailThread.id })
        } catch (error) {
            alert(error.message);
            dispatch(toggleDownVoteCommentActionCreator({ commentId: commentId, userId: authUser.id }))
        }
...
function asyncNeutralizeVoteThreadComment(commentId) {
    return async (dispatch, getState) => {
        const { authUser, detailThread } = getState();
        dispatch(toggleNeutralizeVoteCommentActionCreator({ userId: authUser.id, commentId }))

        try {
            await api.toggleNeutralizeVoteComment({ commentId: commentId, threadId: detailThread.id });
        } catch (error) {
            alert(error.message);
            dispatch(toggleNeutralizeVoteCommentActionCreator({ commentId: commentId, userId: authUser.id }))
        }
    }
}
...
function asyncAddComment({ content }) {
    return async (dispatch, getState) => {
        const { authUser, detailThread } = getState();
        try {
            const comment = await api.addComment({ threadId: detailThread.id, content })
            dispatch(addCommentActionCreator({ comment: comment, userId: authUser.id }))
        } catch(error) {
            alert(error.message);
        }
    }
